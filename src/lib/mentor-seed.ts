// Seeded mentor directory for Mentor Match.
//
// These are transparent DEMO profiles, not live people. They exist so the
// matching, scheduling, prep and follow-up flows are usable end to end before a
// real volunteer-mentor programme is wired in. Every row carries `is_demo: true`
// and the UI labels the directory as demo data.
//
// When a real mentor intake (community partner, sponsor programme, internal
// volunteer form) is connected, insert rows in exactly this shape with
// `is_demo: false` and nothing else in the app has to change.

export type MentorSlot = {
  /** 0 = Sunday … 6 = Saturday */
  dow: number;
  /** Local hour, 24h */
  hour: number;
  minutes: number;
};

export type SeedMentor = {
  external_id: string;
  full_name: string;
  title: string;
  company: string;
  bio: string;
  expertise: string[];
  role_track: string;
  seniority: string;
  languages: string[];
  city: string | null;
  country: string | null;
  lat: number | null;
  lng: number | null;
  meeting_pref: "remote" | "in_person" | "either";
  availability_summary: string;
  topics: string[];
  community: string | null;
  years_experience: number;
  slots: MentorSlot[];
};

export const MENTOR_SEED: SeedMentor[] = [
  {
    external_id: "demo:mentor-lea-brunner",
    full_name: "Lea Brunner",
    title: "Staff Frontend Engineer",
    company: "Swisscom",
    bio: "Started in graphic design, retrained into frontend at 29, and now leads the design-system guild. I know exactly what it feels like to be the least technical person in the room and to grow out of that.",
    expertise: ["React", "TypeScript", "Design Systems", "Accessibility", "CSS"],
    role_track: "Frontend Engineering",
    seniority: "Staff",
    languages: ["German", "English"],
    city: "Zurich",
    country: "CH",
    lat: 47.3769,
    lng: 8.5417,
    meeting_pref: "either",
    availability_summary: "Tuesday and Thursday evenings, 45–60 min",
    topics: ["Career switching", "Portfolio review", "Frontend interviews", "Imposter feelings"],
    community: "Women in Tech Switzerland",
    years_experience: 11,
    slots: [
      { dow: 2, hour: 18, minutes: 45 },
      { dow: 4, hour: 19, minutes: 60 },
    ],
  },
  {
    external_id: "demo:mentor-nadia-haddad",
    full_name: "Nadia Haddad",
    title: "Senior Data Scientist",
    company: "Roche",
    bio: "I came from biology, taught myself Python on the night shift, and now build models in a regulated environment. Ask me about learning stats without a maths degree and about being taken seriously in review meetings.",
    expertise: ["Python", "Machine Learning", "SQL", "Statistics", "Data Visualization"],
    role_track: "Data Science",
    seniority: "Senior",
    languages: ["French", "English", "Arabic"],
    city: "Basel",
    country: "CH",
    lat: 47.5596,
    lng: 7.5886,
    meeting_pref: "either",
    availability_summary: "Wednesday lunchtimes and Saturday mornings",
    topics: ["Breaking into data", "Portfolio projects", "Technical storytelling", "Salary talks"],
    community: "PyLadies Basel",
    years_experience: 8,
    slots: [
      { dow: 3, hour: 12, minutes: 45 },
      { dow: 6, hour: 10, minutes: 60 },
    ],
  },
  {
    external_id: "demo:mentor-priya-raman",
    full_name: "Priya Raman",
    title: "Engineering Manager",
    company: "Google",
    bio: "Twelve years shipping backend systems, five of them managing. I mentor women who are being told they are 'not quite ready' — usually they are, and we build the evidence together.",
    expertise: ["Leadership", "System Design", "Go", "Kubernetes", "Career Growth"],
    role_track: "Engineering Leadership",
    seniority: "Manager",
    languages: ["English", "Hindi"],
    city: "Zurich",
    country: "CH",
    lat: 47.3769,
    lng: 8.5417,
    meeting_pref: "remote",
    availability_summary: "Monday mornings, 30 min focused calls",
    topics: ["Promotion cases", "Negotiation", "Managing up", "Interview strategy"],
    community: "Lean In Circles Zurich",
    years_experience: 12,
    slots: [
      { dow: 1, hour: 8, minutes: 30 },
      { dow: 1, hour: 9, minutes: 30 },
    ],
  },
  {
    external_id: "demo:mentor-sofia-marchetti",
    full_name: "Sofia Marchetti",
    title: "Cloud Platform Engineer",
    company: "Zühlke",
    bio: "Consultant turned platform engineer. I spend my days in AWS, Terraform and CI pipelines, and my evenings convincing career switchers that infrastructure is learnable in public.",
    expertise: ["AWS", "Terraform", "Docker", "CI/CD", "Cloud Architecture"],
    role_track: "Cloud & DevOps",
    seniority: "Senior",
    languages: ["Italian", "German", "English"],
    city: "Lugano",
    country: "CH",
    lat: 46.0037,
    lng: 8.9511,
    meeting_pref: "remote",
    availability_summary: "Thursday evenings, fortnightly",
    topics: ["Cloud certifications", "Homelab projects", "First DevOps role"],
    community: "Cloud Native Switzerland",
    years_experience: 9,
    slots: [
      { dow: 4, hour: 18, minutes: 60 },
      { dow: 2, hour: 20, minutes: 45 },
    ],
  },
  {
    external_id: "demo:mentor-mira-okonkwo",
    full_name: "Mira Okonkwo",
    title: "Product Manager",
    company: "Revolut",
    bio: "I moved from customer support to product in three years by writing down everything I noticed. If you are trying to prove product instincts without a product title yet, that is my favourite problem.",
    expertise: ["Product Management", "User Research", "Analytics", "Roadmapping", "SQL"],
    role_track: "Product",
    seniority: "Mid-senior",
    languages: ["English"],
    city: "London",
    country: "GB",
    lat: 51.5072,
    lng: -0.1276,
    meeting_pref: "remote",
    availability_summary: "Friday afternoons, 45 min",
    topics: ["Into product without a PM title", "Case interviews", "Stakeholder pushback"],
    community: "Women in Product",
    years_experience: 7,
    slots: [
      { dow: 5, hour: 15, minutes: 45 },
      { dow: 5, hour: 16, minutes: 45 },
    ],
  },
  {
    external_id: "demo:mentor-anouk-devries",
    full_name: "Anouk de Vries",
    title: "Backend Engineer",
    company: "Adyen",
    bio: "Bootcamp graduate in 2019, now on a payments core team. I remember every stage of the 'am I actually an engineer yet' spiral and I will walk you through yours honestly.",
    expertise: ["Java", "Python", "APIs", "Databases", "Testing"],
    role_track: "Backend Engineering",
    seniority: "Mid",
    languages: ["Dutch", "English"],
    city: "Amsterdam",
    country: "NL",
    lat: 52.3676,
    lng: 4.9041,
    meeting_pref: "remote",
    availability_summary: "Tuesday evenings, weekly",
    topics: ["Junior job search", "Code review nerves", "Bootcamp to first role"],
    community: "Codam Alumni",
    years_experience: 6,
    slots: [
      { dow: 2, hour: 19, minutes: 45 },
      { dow: 3, hour: 19, minutes: 45 },
    ],
  },
  {
    external_id: "demo:mentor-yara-fischer",
    full_name: "Yara Fischer",
    title: "UX Engineer",
    company: "On Running",
    bio: "I sit between design and code, which means I have opinions about both. Bring me a portfolio, a half-built project or a job ad you feel unqualified for.",
    expertise: ["React", "Figma", "Accessibility", "Design Systems", "Prototyping"],
    role_track: "Design Engineering",
    seniority: "Senior",
    languages: ["German", "English"],
    city: "Zurich",
    country: "CH",
    lat: 47.3769,
    lng: 8.5417,
    meeting_pref: "in_person",
    availability_summary: "Wednesday after work, coffee in Zurich",
    topics: ["Portfolio review", "Design-to-code", "Freelance to employed"],
    community: "Ladies that UX Zurich",
    years_experience: 8,
    slots: [
      { dow: 3, hour: 17, minutes: 60 },
      { dow: 3, hour: 18, minutes: 60 },
    ],
  },
  {
    external_id: "demo:mentor-tessa-lindqvist",
    full_name: "Tessa Lindqvist",
    title: "Security Engineer",
    company: "Proton",
    bio: "Former sysadmin, now application security. I mentor women entering security who worry they are 'not hacker enough' — the job is mostly patience and good questions.",
    expertise: ["Security", "Linux", "Python", "Threat Modeling", "Cryptography"],
    role_track: "Security",
    seniority: "Senior",
    languages: ["Swedish", "English", "German"],
    city: "Geneva",
    country: "CH",
    lat: 46.2044,
    lng: 6.1432,
    meeting_pref: "remote",
    availability_summary: "Monday evenings, monthly deep dives",
    topics: ["Into security", "Certifications worth it", "CTFs without fear"],
    community: "OWASP Geneva",
    years_experience: 10,
    slots: [
      { dow: 1, hour: 19, minutes: 60 },
      { dow: 1, hour: 20, minutes: 45 },
    ],
  },
  {
    external_id: "demo:mentor-ines-moreau",
    full_name: "Inès Moreau",
    title: "Data Engineer",
    company: "Nestlé",
    bio: "Pipelines, warehouses and the unglamorous glue that makes analytics real. I came from finance reporting, so I am fluent in 'my current job is not technical enough'.",
    expertise: ["SQL", "Python", "dbt", "Airflow", "Data Modeling"],
    role_track: "Data Engineering",
    seniority: "Senior",
    languages: ["French", "English"],
    city: "Lausanne",
    country: "CH",
    lat: 46.5197,
    lng: 6.6323,
    meeting_pref: "either",
    availability_summary: "Thursday lunchtimes, 30 min check-ins",
    topics: ["Analytics to engineering", "Learning plans", "Interview take-homes"],
    community: "Women in Data Romandie",
    years_experience: 9,
    slots: [
      { dow: 4, hour: 12, minutes: 30 },
      { dow: 4, hour: 13, minutes: 30 },
    ],
  },
  {
    external_id: "demo:mentor-ruth-abebe",
    full_name: "Ruth Abebe",
    title: "QA Lead",
    company: "SIX Group",
    bio: "Testing taught me systems thinking better than any course. I help people turn 'detail-oriented' into a technical career, and I am blunt about what teams actually reward.",
    expertise: ["Test Automation", "Playwright", "TypeScript", "Quality Strategy"],
    role_track: "Quality Engineering",
    seniority: "Lead",
    languages: ["English", "German"],
    city: "Zurich",
    country: "CH",
    lat: 47.3769,
    lng: 8.5417,
    meeting_pref: "either",
    availability_summary: "Tuesday mornings before standup",
    topics: ["QA to SDET", "Automation portfolio", "Returning after a break"],
    community: "Women in Tech Switzerland",
    years_experience: 13,
    slots: [
      { dow: 2, hour: 8, minutes: 30 },
      { dow: 5, hour: 8, minutes: 45 },
    ],
  },
  {
    external_id: "demo:mentor-clara-vogel",
    full_name: "Clara Vogel",
    title: "AI Engineer",
    company: "DeepJudge",
    bio: "Small startup, large models. I went from research assistant to shipping LLM features and I will happily tell you which parts of the hype are worth your evenings.",
    expertise: ["Python", "Machine Learning", "LLMs", "APIs", "Prompt Engineering"],
    role_track: "AI / ML Engineering",
    seniority: "Mid-senior",
    languages: ["German", "English"],
    city: "Zurich",
    country: "CH",
    lat: 47.3769,
    lng: 8.5417,
    meeting_pref: "remote",
    availability_summary: "Sunday evenings, fortnightly",
    topics: ["AI projects that impress", "Startup vs corporate", "Research to industry"],
    community: "Zurich ML Meetup",
    years_experience: 5,
    slots: [
      { dow: 0, hour: 18, minutes: 45 },
      { dow: 0, hour: 19, minutes: 45 },
    ],
  },
  {
    external_id: "demo:mentor-elif-kaya",
    full_name: "Elif Kaya",
    title: "Technical Program Manager",
    company: "Microsoft",
    bio: "I organise engineering chaos for a living and I coach on visibility: how to make your work legible to the people who decide about your career. Also a Teams power user, for better or worse.",
    expertise: ["Program Management", "Stakeholder Management", "Agile", "Communication"],
    role_track: "Program Management",
    seniority: "Senior",
    languages: ["Turkish", "English", "German"],
    city: "Munich",
    country: "DE",
    lat: 48.1351,
    lng: 11.582,
    meeting_pref: "remote",
    availability_summary: "Friday mornings, 30 min",
    topics: ["Visibility at work", "Performance reviews", "Non-coding tech careers"],
    community: "Microsoft Women's Network",
    years_experience: 11,
    slots: [
      { dow: 5, hour: 9, minutes: 30 },
      { dow: 5, hour: 10, minutes: 30 },
    ],
  },
  {
    external_id: "demo:mentor-nadia-hofer",
    full_name: "Nadia Hofer",
    title: "Senior UX Designer",
    company: "Migros",
    bio: "I came from architecture and now design retail apps used by millions of weekly shoppers. I coach on portfolios that show thinking, not just pretty screens.",
    expertise: ["UX Design", "User Research", "Figma", "Prototyping", "Design Systems"],
    role_track: "UX & Product Design",
    seniority: "Senior",
    languages: ["German", "English"],
    city: "Zurich",
    country: "CH",
    lat: 47.3769,
    lng: 8.5417,
    meeting_pref: "either",
    availability_summary: "Wednesday evenings, 45 min",
    topics: ["Portfolio review", "Design critique", "Switching from a creative field"],
    community: "Women in Digital Switzerland",
    years_experience: 10,
    slots: [
      { dow: 3, hour: 18, minutes: 45 },
      { dow: 3, hour: 19, minutes: 45 },
    ],
  },
  {
    external_id: "demo:mentor-corinne-baumgartner",
    full_name: "Corinne Baumgartner",
    title: "Embedded Systems Engineer",
    company: "a Swiss medtech firm",
    bio: "Firmware for medical devices: C, RTOS, and a lot of oscilloscope time. I mentor people who like hardware but were told software pays better.",
    expertise: ["Embedded C", "RTOS", "Microcontrollers", "PCB Design", "Debugging"],
    role_track: "Embedded & Hardware",
    seniority: "Senior",
    languages: ["German", "English"],
    city: "Bern",
    country: "CH",
    lat: 46.948,
    lng: 7.4474,
    meeting_pref: "remote",
    availability_summary: "Monday evenings, fortnightly",
    topics: ["Firmware first job", "Hardware portfolios", "Regulated industries"],
    community: null,
    years_experience: 13,
    slots: [
      { dow: 1, hour: 18, minutes: 60 },
      { dow: 1, hour: 20, minutes: 45 },
    ],
  },
  {
    external_id: "demo:mentor-alina-petrova",
    full_name: "Alina Petrova",
    title: "Robotics Software Engineer",
    company: "ANYbotics",
    bio: "ROS, control loops and field testing in the rain. I talk honestly about how physical robotics differs from the simulations you build while learning.",
    expertise: ["ROS", "C++", "Python", "Computer Vision", "Control Systems"],
    role_track: "Robotics",
    seniority: "Mid-senior",
    languages: ["Russian", "English"],
    city: "Zurich",
    country: "CH",
    lat: 47.3769,
    lng: 8.5417,
    meeting_pref: "either",
    availability_summary: "Thursday evenings, 60 min",
    topics: ["Robotics portfolios", "Research to industry", "Simulation vs hardware"],
    community: null,
    years_experience: 8,
    slots: [
      { dow: 4, hour: 18, minutes: 60 },
      { dow: 6, hour: 10, minutes: 60 },
    ],
  },
  {
    external_id: "demo:mentor-fatima-benali",
    full_name: "Fatima Benali",
    title: "Bioinformatics Scientist",
    company: "a Basel life-sciences company",
    bio: "Biology degree, self-taught Python, now building genomics pipelines. If you are trying to cross from a science degree into computational work, that is my whole story.",
    expertise: ["Python", "R", "Genomics", "Nextflow", "Statistics"],
    role_track: "Bioinformatics",
    seniority: "Senior",
    languages: ["French", "English", "Arabic"],
    city: "Basel",
    country: "CH",
    lat: 47.5596,
    lng: 7.5886,
    meeting_pref: "remote",
    availability_summary: "Tuesday lunchtimes, 45 min",
    topics: ["Science to tech", "Computational portfolios", "PhD or industry"],
    community: null,
    years_experience: 11,
    slots: [
      { dow: 2, hour: 12, minutes: 45 },
      { dow: 2, hour: 13, minutes: 45 },
    ],
  },
  {
    external_id: "demo:mentor-julia-keller",
    full_name: "Julia Keller",
    title: "IT Systems Administrator",
    company: "a cantonal hospital",
    bio: "Helpdesk to systems administration in five years. I mentor people who start in support and are told that is not a 'real' tech career — it absolutely is a route.",
    expertise: ["Windows Server", "Networking", "Active Directory", "Scripting", "IT Support"],
    role_track: "IT Operations",
    seniority: "Mid-senior",
    languages: ["German", "English"],
    city: "Lucerne",
    country: "CH",
    lat: 47.0502,
    lng: 8.3093,
    meeting_pref: "either",
    availability_summary: "Wednesday mornings, 30 min",
    topics: ["Support to sysadmin", "IT certifications", "Shift work and study"],
    community: null,
    years_experience: 9,
    slots: [
      { dow: 3, hour: 8, minutes: 45 },
      { dow: 3, hour: 9, minutes: 45 },
    ],
  },
  {
    external_id: "demo:mentor-marta-silva",
    full_name: "Marta Silva",
    title: "Senior Technical Writer",
    company: "a Geneva SaaS company",
    bio: "Ex-journalist who now documents APIs. Writing is a legitimate way into tech and it pays — I will show you what a hiring sample looks like.",
    expertise: ["Technical Writing", "API Documentation", "Markdown", "Git", "Information Architecture"],
    role_track: "Technical Writing",
    seniority: "Senior",
    languages: ["Portuguese", "English", "French"],
    city: "Geneva",
    country: "CH",
    lat: 46.2044,
    lng: 6.1432,
    meeting_pref: "remote",
    availability_summary: "Friday afternoons, 45 min",
    topics: ["Writing samples", "Non-coding tech roles", "Freelance to employed"],
    community: null,
    years_experience: 12,
    slots: [
      { dow: 5, hour: 14, minutes: 45 },
      { dow: 5, hour: 15, minutes: 45 },
    ],
  },
  {
    external_id: "demo:mentor-hana-yildiz",
    full_name: "Hana Yildiz",
    title: "Mobile Engineer",
    company: "Twint",
    bio: "iOS and Android for a payments app used daily across Switzerland. I coach on shipping a real app to the store as your strongest interview asset.",
    expertise: ["Swift", "Kotlin", "Mobile Architecture", "App Store Release", "Testing"],
    role_track: "Mobile Engineering",
    seniority: "Senior",
    languages: ["Turkish", "German", "English"],
    city: "Zurich",
    country: "CH",
    lat: 47.3769,
    lng: 8.5417,
    meeting_pref: "remote",
    availability_summary: "Tuesday evenings, 45 min",
    topics: ["Shipping your first app", "Mobile interviews", "Side project scope"],
    community: null,
    years_experience: 10,
    slots: [
      { dow: 2, hour: 19, minutes: 45 },
      { dow: 4, hour: 19, minutes: 45 },
    ],
  },
  {
    external_id: "demo:mentor-eva-lindqvist",
    full_name: "Eva Lindqvist",
    title: "Solutions Engineer",
    company: "a European cloud vendor",
    bio: "Half engineer, half explainer. If you like technology and people equally and nobody has told you that is a job, come talk to me.",
    expertise: ["Solution Architecture", "APIs", "SQL", "Demos", "Customer Discovery"],
    role_track: "Solutions & Pre-sales Engineering",
    seniority: "Senior",
    languages: ["Swedish", "English", "German"],
    city: "Zug",
    country: "CH",
    lat: 47.1662,
    lng: 8.5155,
    meeting_pref: "either",
    availability_summary: "Monday afternoons, 45 min",
    topics: ["Technical but people-facing roles", "Interview demos", "Commercial tech careers"],
    community: null,
    years_experience: 12,
    slots: [
      { dow: 1, hour: 14, minutes: 45 },
      { dow: 1, hour: 16, minutes: 45 },
    ],
  },
  {
    external_id: "demo:mentor-rina-desai",
    full_name: "Rina Desai",
    title: "Site Reliability Engineer",
    company: "a Zurich fintech",
    bio: "On-call, incident reviews and making systems boring on purpose. I mentor on breaking into infrastructure without a CS degree.",
    expertise: ["Kubernetes", "Linux", "Observability", "Go", "Incident Response"],
    role_track: "SRE & Infrastructure",
    seniority: "Senior",
    languages: ["English", "Gujarati"],
    city: "Zurich",
    country: "CH",
    lat: 47.3769,
    lng: 8.5417,
    meeting_pref: "remote",
    availability_summary: "Thursday mornings, 45 min",
    topics: ["Into infrastructure", "Homelab to hired", "On-call reality"],
    community: "Cloud Native Switzerland",
    years_experience: 11,
    slots: [
      { dow: 4, hour: 8, minutes: 45 },
      { dow: 4, hour: 9, minutes: 45 },
    ],
  },
  {
    external_id: "demo:mentor-lucie-favre",
    full_name: "Lucie Favre",
    title: "Data Analyst",
    company: "a Lausanne climate-tech startup",
    bio: "Spreadsheets to dashboards to decisions. Analytics is the most forgiving door into data work and I will help you pick projects that prove it.",
    expertise: ["SQL", "Power BI", "Python", "Data Visualisation", "Excel"],
    role_track: "Data Analytics",
    seniority: "Mid",
    languages: ["French", "English"],
    city: "Lausanne",
    country: "CH",
    lat: 46.5197,
    lng: 6.6323,
    meeting_pref: "either",
    availability_summary: "Wednesday evenings, 45 min",
    topics: ["First analyst role", "Dashboard portfolios", "SQL interviews"],
    community: "Women In Digital Switzerland",
    years_experience: 6,
    slots: [
      { dow: 3, hour: 18, minutes: 45 },
      { dow: 6, hour: 11, minutes: 45 },
    ],
  },
  {
    external_id: "demo:mentor-sabine-roth",
    full_name: "Sabine Roth",
    title: "Accessibility Specialist",
    company: "a Swiss public-sector digital team",
    bio: "I audit services so people can actually use them. Accessibility work sits between design, code and policy, and it is desperately short of people.",
    expertise: ["WCAG", "Accessibility Testing", "HTML", "Screen Readers", "Usability"],
    role_track: "Accessibility",
    seniority: "Senior",
    languages: ["German", "English", "French"],
    city: "Bern",
    country: "CH",
    lat: 46.948,
    lng: 7.4474,
    meeting_pref: "remote",
    availability_summary: "Tuesday afternoons, 45 min",
    topics: ["Accessibility as a career", "Auditing basics", "Public-sector tech"],
    community: null,
    years_experience: 14,
    slots: [
      { dow: 2, hour: 14, minutes: 45 },
      { dow: 2, hour: 16, minutes: 45 },
    ],
  },
  {
    external_id: "demo:mentor-camille-perrin",
    full_name: "Camille Perrin",
    title: "Game Developer",
    company: "an independent studio",
    bio: "Unity, small teams, shipped titles. Games are a hard industry and I would rather tell you the truth early than let you find out after a year of unpaid jam work.",
    expertise: ["Unity", "C#", "Game Design", "3D Maths", "Prototyping"],
    role_track: "Game Development",
    seniority: "Mid-senior",
    languages: ["French", "English"],
    city: "Geneva",
    country: "CH",
    lat: 46.2044,
    lng: 6.1432,
    meeting_pref: "remote",
    availability_summary: "Sunday afternoons, 60 min",
    topics: ["Game portfolios", "Studio hiring", "Jams that count"],
    community: null,
    years_experience: 8,
    slots: [
      { dow: 0, hour: 15, minutes: 60 },
      { dow: 0, hour: 17, minutes: 45 },
    ],
  },
  {
    external_id: "demo:mentor-yara-haddad",
    full_name: "Yara Haddad",
    title: "Network Engineer",
    company: "a Swiss telecoms operator",
    bio: "Routing, switching and the unglamorous plumbing everything else depends on. Certifications genuinely work in this field and I will help you sequence them.",
    expertise: ["Networking", "TCP/IP", "Cisco", "Firewalls", "Automation"],
    role_track: "Networking",
    seniority: "Senior",
    languages: ["Arabic", "English", "French"],
    city: "Geneva",
    country: "CH",
    lat: 46.2044,
    lng: 6.1432,
    meeting_pref: "either",
    availability_summary: "Monday evenings, 45 min",
    topics: ["Network certifications", "Ops to engineering", "Studying while working"],
    community: null,
    years_experience: 13,
    slots: [
      { dow: 1, hour: 18, minutes: 45 },
      { dow: 3, hour: 18, minutes: 45 },
    ],
  },
  {
    external_id: "demo:mentor-tanja-novak",
    full_name: "Tanja Novak",
    title: "Scrum Master & Agile Coach",
    company: "a Zurich insurance group",
    bio: "Former teacher, now coaching delivery teams. If you are organised, calm in conflict and curious about tech, there is a real career here without writing code.",
    expertise: ["Agile", "Scrum", "Facilitation", "Coaching", "Delivery Planning"],
    role_track: "Agile Delivery",
    seniority: "Senior",
    languages: ["Croatian", "German", "English"],
    city: "Zurich",
    country: "CH",
    lat: 47.3769,
    lng: 8.5417,
    meeting_pref: "either",
    availability_summary: "Thursday lunchtimes, 30 min",
    topics: ["Non-coding tech careers", "Teaching to tech", "Certification value"],
    community: null,
    years_experience: 10,
    slots: [
      { dow: 4, hour: 12, minutes: 45 },
      { dow: 4, hour: 13, minutes: 45 },
    ],
  },
];
